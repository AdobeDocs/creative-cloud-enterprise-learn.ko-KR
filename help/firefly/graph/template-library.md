...
* [**브랜딩 시각화**](/help/firefly/graph/templates/branding-visualization.md) — 이 그래프 템플릿에서 제품 장면을 시각화하는 방법을 알아보십시오. 브랜드 가이드라인이나 로고 및 색상 팔레트로 피드 및 그래프를 사용하면 한 번의 실행으로 정적 키 아트와 짧은 모션 패스가 모두 출력되므로 두 포맷 모두 시각적으로 정렬된 상태를 유지할 수 있습니다.
